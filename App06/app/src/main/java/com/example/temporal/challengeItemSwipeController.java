...
public class challengeItemSwipeController extends ItemTouchHelper.Callback {
    private challengeItemSwipeListener listener;
    ...
    public challengeItemSwipeController(challengeItemSwipeListener listener) {
    ...
    private void drawButtons(Canvas c, RecyclerView.ViewHolder viewHolder) {
        ...
        if (buttonsShowedState == ButtonsState.RIGHT_VISIBLE) {
            RectF rightButton = new RectF(
                    itemView.getRight() - buttonWidthWithOutPadding, itemView.getTop() + 30, itemView.getRight() - 30, itemView.getBottom() - 30);
    ...
    private void setTouchUpListener(final Canvas c, final RecyclerView recyclerView, final RecyclerView.ViewHolder viewHolder, final float dX, final float dY, final int actionState, final boolean isCurrentlyActive) {
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                challengeItemSwipeController.super.onChildDraw(c, recyclerView, viewHolder, 0F, dY, actionState, isCurrentlyActive);
                recyclerView.setOnTouchListener(new View.OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {
                        return false;
